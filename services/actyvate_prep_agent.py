...
if __name__ == "__main__":
    ...
    flow_file = os.path.join(os.path.dirname(__file__), "..", "app", "workflows", "actyvate_prep_call_flow.json")
    flow_file = os.path.normpath(flow_file)
    if not os.path.exists(flow_file):
        # try repository path
        flow_file = os.path.join(os.getcwd(), "app", "workflows", "actyvate_prep_call_flow.json")

    agent = PrepCallAgent(flow_file)
    agent.start({
    ...
    # simulate a successful permission-to-continue, then identity confirmed
    agent.transition({"permission_to_continue": True})
    print("Now at:", agent.current_node)
